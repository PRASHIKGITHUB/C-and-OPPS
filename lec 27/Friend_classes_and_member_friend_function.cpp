/*Here we are using private members of another class's
using different class's function*/
#include <iostream>
using namespace std;
// Forward Declaration
class complex;
class calculator
{
public: 
    int add(int a, int b)
    {
        return a + b;
    }

    int sumRealcomplex(complex, complex);
    int sumcomcomplex(complex, complex);
};
class complex
{
    int a, b;
    //INdividually declaring functions as friends
    friend int calculator::sumRealcomplex(complex o1, complex o2);
    friend int calculator::sumcomcomplex(complex o1, complex o2);
    //Aliter: Declaring the entire Calculator calss as friend
    friend class calculator;

    //either you can make some functions as a friend or else you can make an entire class as a friend that means every function can access private memebers

public:
    void setnumber(int n1, int n2)
    {
        a = n1;
        b = n2;
    }
    void printnumber()
    {
        cout << "your number is " << a << " + " << b << "i" << endl;
    }
};
int calculator::sumRealcomplex(complex o1, complex o2)
{
    return (o1.a + o2.a);
}
int calculator::sumcomcomplex(complex o1, complex o2)
{
    return (o1.b + o2.b);
}
int main()
{
    complex o1, o2;
    o1.setnumber(1, 4);
    o2.setnumber(5, 7);
    o1.printnumber();
    o2.printnumber();
    calculator calc;
    int result = calc.sumRealcomplex(o1, o2);
    cout << "The sum of real part of o1 and o2 is " << result << endl;
    result = calc.sumcomcomplex(o1, o2);
    cout << "The sum of complex part of o1 and o2 is " << result << endl;

    return 0;
}